...
ap.add_argument('-nb_epoch', type=int, default=50)
...
if __name__ == '__main__':
    ...
    X, X_vocab_len, X_word_to_ix, X_ix_to_word, y, y_vocab_len, y_word_to_ix, y_ix_to_word = load_data("train.enc","train.dec","test.enc","test.dec", MAX_LEN, VOCAB_SIZE)
    ...
    # Performing test if we chose test mode
        ...
        if len(saved_weights) == 0:
            ...
            print("Welcome!")
            while True:
                output_file = open(os.path.join('', 'output_convo.txt'), 'a+')
                ...
                X = [text_to_word_sequence(x)[::-1] for x in str(line).split('\n') if len(x) > 0 and len(x) <= X_max_len]
                ...
                X_test = pad_sequences(X, maxlen=X_max_len, dtype='int32')
                predictions = np.argmax(model.predict(X_test), axis=2)
                sequences = []
                for prediction in predictions:              
                    sequence = ' '.join([y_ix_to_word[index] for index in prediction if index > 0])
                    print(sequence)
                    output_file.write('BOT ++++ ' + sequence + '\n')
            
            output_file.write('=============================================\n')
            output_file.close()
            
            '''
            X_test = load_test_data('convo', X_word_to_ix, MAX_LEN)
